Page({
  ...
  chooseImage(e) {
    const { type, source } = e.currentTarget.dataset; // 'source' or 'reference'; 'camera' or 'album'
    wx.chooseImage({
      count: 1,
      ...
      success: (res) => {
        ...
        if (type === 'source') {
          this.setData({ sourceSrc: tempFilePath, errorMsg: '' });
        } else if (type === 'reference') {
          this.setData({ referenceSrc: tempFilePath, errorMsg: '' });
        }
      },
      fail: (err) => {
  ...
  startGeneration() {
    if (!this.data.sourceSrc || !this.data.referenceSrc) {
      this.setData({ errorMsg: '请先上传源姿势图像和参考人物图像' });
      wx.showToast({
        title: '请先上传图片',
        icon: 'none'
      });
    ...
    // 使用JSON+Base64方式发送请求
    const apiUrl = 'https://5fqtlb6nrk8-ghost.gear-c1.openbayes.net/pose-transfer';
    ...
    Promise.all([
      ...
      new Promise((resolve, reject) => {
        fileSystemManager.readFile({
          filePath: this.data.referenceSrc,
          encoding: 'base64',
          success: res => resolve(res.data),
          fail: reject
        });
      })
    ]).then(([sourceBase64, referenceBase64]) => {
      // 使用FormData方式发送请求，适应服务器端的Form参数期望
      const formData = {
